    ...
    return (
        <div className='main'>
            <div>
                ...
                {redirect.redirect ? <Redirect to={`news/${redirect.index}`}/>: void 0}
            </div>
        </div>
    )
}
...
class SimpleSlider extends Component {
    ...
    render() {
        return (
                        ...
                        this.state.newsData.map((elem, index) => {
                            return (
                                <div className='slider__elem' key={index}>
                                    ...
                                    <button className='slider__elem-detail' onClick={() => this.props.watchDetailsNewsElem(index)}>
            ...
            </div> : !this.state.error ?<p className='info'>Загрузка данных...</p> : <p className='info'>Чо-то пошло не так...</p>
        );
    }
}
