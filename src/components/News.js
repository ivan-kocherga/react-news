...
export default function News() {
    ...
    let urlParams = helpFunc.getQueryStringParams(document.location.search).request
    ...
    useEffect(() => {
        ...
        const sub = store.subscribe(() => {
            setSearchInputValue(
                store.getState().value === 'ua' ? 'Украина'
                    : store.getState().value === 'ru' ? 'Росия'
                    : store.getState().value === 'us' ? 'Америка'
                        : 'Неизвестно'
            )
            reloadNews()
        })
        ...
        setSearchInputValue(
            NewsAPI.country === 'ua' ? 'Украина'
                : NewsAPI.country === 'ru' ? 'Росия'
                : NewsAPI.country === 'us' ? 'Америка'
                    : 'Неизвестно'
        )
    ...
    function updateNewsParams(str){
        newsApi.getSearchNews(str).then(res => {
    ...
    return (
        <div className='news'>
            <div className="news-wrapper">
                <div className="news__title">
                    ...
                    <div className='news__title-search'>
                        <input type="text" value={value} onChange={e => setValue(e.target.value)} placeholder='Какие новости вы хотите найти?'/>
                ...
                </div>
                    {
                        news.length > 0 ?
                                    ...
                                    news.map((elem, index) => {
                                        return (
                                            <div className='news__elem slider-elem' key={index}>
                                                <img className={'img-'+ index} src={elem.urlToImage} onError={(e) => helpFunc.imgError(e)}/>
                            ...
                            : notNews ? <p className='load'>Новостей не найдено...</p> : <p className='load'>Загрузка...</p>
            ...
            {
                index.redirect && value ? <Redirect to={ `news/${index.index}?lastQuestion=${value.trim()}`}/>:
                index.redirect ? <Redirect to={ `news/${index.index}?county=${newsApi.country}`}/>
                : void 0
            }
            {Boolean(params) ?  <Redirect to={`news?request=${params}`}/> : <Redirect to={'news'}/>}
        </div>
    )
}